...
description: |
  用 HTML/CSS/JS/React 构建高品质视觉作品（网页、落地页、仪表盘、交互原型、幻灯片、动画演示、数据可视化等）。
  当用户需要视觉、交互或前端相关的产出时使用此技能：
  - 创建网页、落地页、仪表盘、营销页面
  - 构建交互原型或 UI 模型（含设备框架）
  - 构建 HTML 幻灯片/演示文稿
  - 创建 CSS/JS 动画或时间轴驱动的动画演示
  - 将设计稿、截图或 PRD 转化为交互实现
  - 数据可视化（Chart.js / D3 等）
  - 设计系统 / UI Kit 探索
  即使用户没有明确说"HTML"或"网页"，只要意图是产出视觉、交互或展示性内容，都适用此技能。
  不适用：纯后端逻辑、CLI 工具、数据处理脚本、非视觉代码任务、命令行调试。
---
