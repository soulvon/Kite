...
(function () {
	...
	// 解决死锁：bridge 未就绪时补丁端本地改的 settings 无法回写 enh-settings.json，
	// 重启 Windsurf 后 injected.__WS_BETTER_INJECTED_SETTINGS__ 会覆盖回旧值。
	// 通过队列暂存 + bridge 就绪时回放 → 持久化到 enh-settings.json + 重写 workbench.html。
	const PENDING_ENH_PATCH_KEY = 'ws-pending-enh-patch';
	function enqueuePendingEnhPatch(patch) {
		if (!patch || typeof patch !== 'object') return;
		try {
			const raw = localStorage.getItem(PENDING_ENH_PATCH_KEY);
			const list = raw ? JSON.parse(raw) : [];
			list.push({ ts: Date.now(), patch });
			while (list.length > 50) list.shift();
			localStorage.setItem(PENDING_ENH_PATCH_KEY, JSON.stringify(list));
			console.log('[ws-better][enh-patch] 入队: ' + Object.keys(patch).join(','));
		} catch {}
	}
	// 注意：不能用 bridgePostResult，它静默吞所有错误（fetch 失败也不抛），
	// 这里需要明确知道扩展端是否真的收到，否则误清队列会导致数据丢失。
	async function flushPendingEnhPatch() {
		let list;
		try {
			const raw = localStorage.getItem(PENDING_ENH_PATCH_KEY);
		...
		// bridge 没配置端口/token 直接 return，下次就绪自动再调
		if (typeof getBridgeUrl !== 'function' || typeof getBridgeHeaders !== 'function') return;
		const base = getBridgeUrl();
		if (!base) return;
		// 多条 patch 后入覆盖前入（用户的最新意图为准）
		const merged = {};
		for (const item of list) {
			if (item && item.patch) Object.assign(merged, item.patch);
		}
		if (Object.keys(merged).length === 0) {
			try { localStorage.removeItem(PENDING_ENH_PATCH_KEY); } catch {}
			return;
		}
		console.log('[ws-better][enh-patch] 回放 → 扩展: ' + Object.keys(merged).join(','));
		let ok = false;
		try {
			...
			const res = await fetch(base + '/result', {
				...
				body: JSON.stringify({ type: 'enh-settings-patch', patch: merged, ts: Date.now() }),
				signal: ctrl.signal,
			});
			clearTimeout(tid);
			ok = !!(res && res.ok);
			if (!ok) console.warn('[ws-better][enh-patch] 回放 HTTP 失败 status=' + (res && res.status));
		} catch (e) {
			console.warn('[ws-better][enh-patch] 回放网络失败:', e && e.message || e);
		}
		// 关键：只有扩展端真正 ack（HTTP 200）才清队列，失败时保留下次自动重试
		if (ok) {
			try { localStorage.removeItem(PENDING_ENH_PATCH_KEY); } catch {}
			console.log('[ws-better][enh-patch] ✓ 已持久化，队列清空');
		} else {
			console.log('[ws-better][enh-patch] 队列保留，下次 bridge 就绪重试');
		}
	}
...
`;
	...
	// v8.5.6：扩展词汇表
	const MODEL_DESC_RE = /\b(for|via|using|with|agent|coding|powered|based|built|available|supported|requires?|enables?|provides?|settings|apply|saved|balance|draw|from|describe|task|detect|reload|enable|disable|when|will|this|that|default|click|select|enter|input|edit|change|update|reloads|please|note|tip|hint|info|warning|error|failed|success|complete|reopen|restart|reset|clear|remove|delete|add|create|new|import|export|download|upload|view|open|close|save|cancel|confirm|run|start|stop|pause|resume|show|hide|display|render|compute|process|handle|manage|configure)\b/i;
	...
	const TRANSLATIONS = new Map([
		...
		// ========== New Devin/Cascade Translations (v7.10.1) ==========
		...
		['Start With History', '带着历史对话开始'],
		['View all Devin shortcuts', '查看所有 Devin 快捷键'],
		['View all Windsurf shortcuts', '查看所有 Windsurf 快捷键'],
		['Current plan', '当前套餐'],
		['Plan ends in 4 days', '计划在 4 天内结束'],
		['Allowed origins', '允许的源'],
		['Browse, install, and manage MCP servers from the Cascade MCP marketplace. Enable or disable servers and individual tools', '从 Cascade MCP 市场浏览、安装和管理 MCP 服务器。启用或禁用服务器及单个工具。'],
		['Open MCP marketplace', '打开 MCP 市场'],
		['Cascade keeps running when you switch conversations. Terminal commands may run in the background depending on your auto execution setting', '当您切换会话时，Cascade 将在后台保持运行。根据您的自动执行设置，终端命令可能会在后台运行。'],
		['Arena mode sessions open in the editor tab for a side-by-side view', '启用后，Arena 模式会话将自动在编辑器标签页中打开以进行并排视图。'],
		['Cascade continues past the invocation limit without prompting; otherwise it stops and asks you to continue', '控制 Cascade 达到调用限制时是否自动继续。开启时，Cascade 无限期继续而无需提示。关闭时，Cascade 在调用限制处停止并询问您是否继续。'],
		['Cascade automatically saves important context as memories; otherwise it saves only when you ask', 'Cascade 会自动将重要上下文保存为记忆；否则仅在您请求时保存'],
		['Cascade detects lint errors from its own edits and may fix them automatically. May increase tool usage', 'Cascade 会检测其自身修改引入的 lint 错误并尝试自动修复。这可能会增加工具使用量。'],
		['Cascade opens browser previews of dev servers it starts, integrating tightly with your workflow', 'Cascade 会为其启动的开发服务器打开浏览器预览，与您的工作流紧密集成。'],
		['Stops the fast context agent from running parallel searches as a subagent', '停止快速上下文智能体作为子代理执行并行搜索。'],
		['Cascade can perform web searches on the open internet. Reading specific URLs is unaffected and runs locally on your machine', 'Cascade 可以在互联网上执行网页搜索。读取特定 URL 的操作不受影响，且在您的本地机器上运行。'],
		['Send explain and fix requests to the open conversation instead of a new one', '将解释和修复请求发送到当前打开的对话中，而不是新建一个对话。'],
		['Let Cascade, tab, and supercomplete view and edit files in .gitignore', '允许 Cascade、Tab 补全和超级补全查看并编辑 .gitignore 中的文件。'],
		['Cascade reads skills from .claude directories (local .claude/skills/ and global ~/.claude/skills/)', 'Cascade 会从 .claude 目录中读取技能（包括本地 .claude/skills/ 和全局 ~/.claude/skills/）。'],
		['Allowlist - only allowlisted origins are auto-fetched', '允许列表 - 仅自动获取允许列表中的源'],
		['Turbo - all web requests are auto-fetched', '极速 - 所有 Web 请求均自动获取'],
		['Origins must include the scheme and port if non-default (e.g., "https://github.com" or "http://localhost:3000").', '如果不是默认端口，源（Origin）必须包含协议和端口（例如 "https://github.com" 或 "http://localhost:3000"）。'],
		['Origins Cascade auto-fetches URLs from', 'Cascade 会自动从中获取 URL 的源列表'],
		['Cascade will auto-run commands in the allowlist and ask for permission for commands in the denylist, with the denylist taking precedence. Add "*" at the end of a command for prefix matching (e.g., "git *" matches all git commands).', 'Cascade 将自动运行允许列表中的命令，并对拒绝列表中的命令请求许可，拒绝列表具有更高优先级。在命令末尾添加 "*" 可进行前缀匹配（例如，"git *" 匹配所有 git 命令）。'],
		['Patterns for commands that are always auto-executed', '始终自动执行的命令的匹配模式'],
		['Patterns for commands that are always denied', '始终拒绝的命令的匹配模式'],
		['Notify when Cascade finishes while running in the background', '当 Cascade 在后台运行完成时发送通知。'],
		['Persistent instructions that shape how Cascade behaves', '塑造 Cascade 行为的持久指令。'],
		['Saved prompts Cascade can follow. Type "/" in Cascade to trigger one', 'Cascade 可以遵循的已保存提示。在 Cascade 中输入 "/" 以触发。'],
		['View and edit memories Cascade generated', '查看和编辑 Cascade 生成的记忆。'],
		['Automatically detect a network proxy. Changing this reloads Devin', '自动检测网络代理。修改此项将重新加载并重启 Devin。'],
		['Maximum files Devin computes embeddings for, ignoring .gitignore and binary files. Higher values may hurt performance; 0 or below means unlimited', 'Devin 计算嵌入的最大文件数（忽略 .gitignore 和二进制文件）。值越大可能会影响性能；输入 0 或以下表示无限制。'],
		['The following workspace folder has a .windsurf/ directory. Would you like to rename it to .devin/?', '以下工作区文件夹包含 .windsurf/ 目录。您想将其重命名为 .devin/ 吗？'],
		['Rename', '重命名'],
		['Client windsurf: connection to server is erroring. Shutting down server.', '客户端 windsurf: 与服务器的连接发生错误。正在关闭服务器。'],
		['Client devin: connection to server is erroring. Shutting down server.', '客户端 devin: 与服务器的连接发生错误。正在关闭服务器。'],
		...
		['Devin uses your clipboard as context for completions', '启用后，Devin 将使用剪贴板内容作为补全的上下文'],
		['Add and update imports with a tab keypress', '按 Tab 键可快速添加并更新导入语句'],
		...
		['Agent marketplace with 174+ AI capabilities. Browse, invoke, and pay for agent s...', '拥有 174+ AI 能力的智能体市场。浏览、调用并支付智能体服务...'],
		['Amp', 'Amp'],
		['ACP wrapper for Amp - the frontier coding agent', 'Amp (前沿编程智能体) 的 ACP 封装器'],
		['Auggie CLI', 'Auggie CLI'],
		['Augment Code\'s powerful software agent, backed by industry-leading context en...', 'Augment Code 的强大软件智能体，由行业领先的上下文增强支持...'],
		['Autohand Code', 'Autohand Code'],
		['Autohand Code - AI coding agent powered by Autohand AI', 'Autohand Code - 基于 Autohand AI 的 AI 编程智能体'],
		['Autonomous coding agent CLI - capable of creating/editing files, running comm...', '自主编程智能体 CLI - 能够创建/编辑文件、运行命令...'],
		['Codebuddy Code', 'Codebuddy Code'],
		['Tencent Cloud\'s official intelligent coding tool', '腾讯云官方智能编码工具'],
		['Cortex Code', 'Cortex Code'],
		['Snowflake\'s Cortex Code coding agent', 'Snowflake 的 Cortex Code 编程智能体'],
		['Corust Agent', 'Corust Agent'],
		['Co-building with a seasoned Rust partner.', '与资深 Rust 伙伴共同构建。'],
		['crow-cli', 'crow-cli'],
		['Minimal ACP Native Coding Agent', '极简 ACP 原生编程智能体'],
		['Cursor\'s coding agent', 'Cursor 的编程智能体'],
		['DeepAgents', 'DeepAgents'],
		['Batteries-included AI coding and general purpose agent powered by LangChain.', '基于 LangChain 的开箱即用 AI 编程及通用智能体。'],
		['DimCode', 'DimCode'],
		['A coding agent that puts leading models at your command.', '听从您指挥、驾驭前沿模型的编程智能体。'],
		['Dirac', 'Dirac'],
		['Reduces API costs by more than 50%, produces better and faster work. Uses Has...', '降低 API 成本超过 50%，产出更好更快的成果。使用 Has...'],
		['Factory Droid', 'Factory Droid'],
		['Factory Droid - AI coding agent powered by Factory AI', 'Factory Droid - 基于 Factory AI 的 AI 编程智能体'],
		['fast-agent', 'fast-agent'],
		['Code and build agents with comprehensive multi-provider support', '具备全面多服务商支持的编程与构建智能体'],
		['Gemini CLI', 'Gemini CLI'],
		['Google\'s official CLI for Gemini', '谷歌官方 Gemini CLI'],
		['GitHub\'s AI pair programmer', 'GitHub 的 AI 结对编程助手'],
		['GLM Agent', 'GLM Agent'],
		['ACP agent powered by Zhipu AI\'s GLM Coding Plan models (glm-5.1, glm-5-turb...', '基于智谱 AI GLM Coding Plan 模型（glm-5.1, glm-5-turb...）的 ACP 智能体'],
		['A local, extensible, open source AI agent that automates engineering tasks', '一个本地、可扩展、开源的 AI 智能体，可自动执行工程任务'],
		['Grok Build', 'Grok Build'],
		['xAI\'s coding agent and CLI', 'xAI 的编程智能体和 CLI'],
		['Junie', 'Junie'],
		['AI Coding Agent by JetBrains', 'JetBrains 推出的 AI 编程智能体'],
		['The open source coding agent', '开源编程智能体'],
		['Kimi CLI', 'Kimi CLI'],
		['Moonshot AI\'s coding assistant', 'Moonshot AI 的编程助手'],
		['Minion Code', 'Minion Code'],
		['An enhanced AI code assistant built on the Minion framework with rich develop...', '基于 Minion 框架构建的增强型 AI 代码助手，具有丰富的开发...'],
		['Mistral Vibe', 'Mistral Vibe'],
		['Mistral\'s open-source coding assistant', 'Mistral 的开源编程助手'],
		['Nova by Compass AI - a fully-fledged software engineer at your command', 'Compass AI 的 Nova - 听从您指挥的完全体软件工程师'],
		['pi ACP', 'pi ACP'],
		['ACP adapter for pi coding agent', 'pi 编程智能体的 ACP 适配器'],
		['Poolside\'s coding agent', 'Poolside 的编程智能体'],
		['Qoder CLI', 'Qoder CLI'],
		['AI coding assistant with agentic capabilities', '具有智能体能力的 AI 编程助手'],
		['Alibaba\'s Qwen coding assistant', '阿里巴巴的通义千问（Qwen）编程助手'],
	...
	['Team settings refresh timed out', '团队设置刷新超时'],
		...
		// v8.5.9 补：chip 翻译（devin 那边已有，windsurf 缺）
		['Model', '模型'],
		['Model: Default', '模型：默认'],
		['Model: ', '模型：'],
		['Effort: ', '推理强度：'],
	...
	function startSimpleContinue() {
		...
		const checkAndSend = async () => {
			...
			try {
				// v7.8.4: 传 'simple' — 用户从 simple 切到其他模式时，已 in-flight 的发送立即中断
				const sent = await sendContinueMessage(null, 'simple');
				if (!sent) {
					// 发送失败，回滚计数
	...
	async function sendInputAndClick(text) {
		...
		// v7.8.3: 写入前先检查 autoRecoveryEnabled（与调用方 executeAfterAction 一致）
		if (!settings.autoRecoveryEnabled) {
			console.log(LOG_PREFIX + '[Recovery] sendInputAndClick 短路: autoRecoveryEnabled=false');
			return false;
		}
		if (!await setInputText(text)) return false;
		// v7.8.3: setInputText 内部有 await 链，写入完成后再次检查 — 用户在 await 期间关开关时清空残留
		if (!settings.autoRecoveryEnabled) {
			console.log(LOG_PREFIX + '[Recovery] sendInputAndClick 短路 @ post-setInput: autoRecoveryEnabled=false');
			const cleanupEl = findInputEl();
			if (cleanupEl) {
				try { cleanupEl.focus(); document.execCommand('selectAll', false, null); document.execCommand('delete', false, null); } catch {}
			}
		...
		markActionClick();
		setTimeout(() => {
			// v7.8.3: setTimeout 闭包内独立短路 — 400ms 内用户可能关闭自动恢复
			if (!settings.autoRecoveryEnabled) {
				console.log(LOG_PREFIX + '[Recovery] sendInputAndClick setTimeout 短路: autoRecoveryEnabled=false');
				return;
			}
			trySendMessage();
		}, 400);
	...
	function showBridgeNotReadyToast() {
		...
		if (disableBtn) disableBtn.onclick = function() {
			...
			//   - bridge 就绪：立即推到扩展端，mergeEnhSettings + ensureEnhancement → 持久化
			//   - bridge 未就绪：留在 localStorage 队列里，下次就绪自动回放
			enqueuePendingEnhPatch({ autoSwitchEnabled: false });
			flushPendingEnhPatch();
	...
	function showRecoveryPrompt(options) {
		...
		function execute() {
			...
			// v7.8.2: 倒计时（5-15s）期间用户可能关掉子开关 → 执行前再过一次 isActionDisabled
			// 修补 v6.6.4 Bug B 的姊妹漏洞：banner 显示时按钮置灰，但倒计时结束自动 execute 时未再检查
			if (isActionDisabled(chosen)) {
				const label = ACTION_LABEL[chosen] || chosen;
				console.log(LOG_PREFIX + '[Recovery] Banner execute 前发现 [' + chosen + '] 已被子开关禁用，取消执行');
				try { showRecoveryNotification('动作「' + label + '」已被关闭，跳过自动执行', 'error'); } catch {}
				// 走 cancel 流程清理 banner UI（避免视觉残留）
	...
	// v7.8.2 修订：
	// - 每个 await 后重新检查 autoRecoveryEnabled / continueMode，避免重试循环（最长 6s）变成僵尸
	async function executeAfterAction(afterAction, opts) {
		opts = opts || {};
		const action = afterAction || 'auto';
		if (action === 'none') return;
		if (action === 'retry-message') {
			// v7.8.2: 真正发送前检查总开关（lastUserMessage 是历史值，关 autoRecovery 后不该重发）
			if (!settings.autoRecoveryEnabled) {
				console.log(LOG_PREFIX + '[Recovery] executeAfterAction 短路: retry-message 时 autoRecoveryEnabled=false');
				return;
			}
		...
		if (action === 'auto') {
			const retryBtn = findRetryButton();
			if (retryBtn) {
				// v7.8.2: 点 retry 按钮前检查 autoRecovery + guardian.autoRetry
				if (!settings.autoRecoveryEnabled || (settings.guardian && settings.guardian.autoRetry === false)) {
					console.log(LOG_PREFIX + '[Recovery] executeAfterAction 短路: auto 路径 retry 按钮被开关禁用');
					return;
				}
		...
		for (let attempt = 1; attempt <= maxAttempts; attempt++) {
			...
			if (!settings.autoRecoveryEnabled) {
				console.log(LOG_PREFIX + '[Recovery] executeAfterAction 短路 @ 循环#' + attempt + ': autoRecoveryEnabled=false');
				return;
			}
			if (settings.continueMode === 'off') {
				console.log(LOG_PREFIX + '[Recovery] executeAfterAction 短路 @ 循环#' + attempt + ': continueMode=off');
				return;
			}
			...
			if (isAIGenerating()) {
				...
				// v7.8.2: 等待 3s 后再次检查 — 用户可能在等待期间关掉开关
				if (!settings.autoRecoveryEnabled || settings.continueMode === 'off') {
					console.log(LOG_PREFIX + '[Recovery] executeAfterAction 短路 @ isAIGenerating-wait#' + attempt);
					return;
				}
				if (isAIGenerating()) {
			...
			if (attempt < maxAttempts) {
				...
				await new Promise(r => setTimeout(r, 3000));
				// v7.8.2: 重试间隔后检查 — 用户在 3s 间隔内关开关也立即中断
				if (!settings.autoRecoveryEnabled || settings.continueMode === 'off') {
					console.log(LOG_PREFIX + '[Recovery] executeAfterAction 短路 @ retry-interval#' + attempt);
					return;
				}
			}
		}
	...
	// v7.8.2 修订：
	// - 每个 await 边界后重新检查 continueMode，避免 in-flight 4-5s 内关开关仍把消息发出
	// v7.8.4 修订：
	// - expectedMode 参数：调用方传入自己期望的模式（'brainless'/'simple'），await 后仅验证未被切走
	//   修复场景：长任务发送中用户点「强制停止」，sendContinueMessage 仅查 'off' 不能拦截 brainless→simple 切换
	...
	async function sendContinueMessage(customText, expectedMode) {
		...
		try {
			...
			for (let attempt = 0; attempt < 3; attempt++) {
				// v7.8.2: 每次尝试前短路检查
				if (_shouldAbortContinueSend('setInput#' + (attempt + 1), expectedMode)) return false;
				if (await setInputText(text)) { setOk = true; break; }
				console.log(LOG_PREFIX + '[sendContinue] setInputText 失败 #' + (attempt + 1) + '/3');
				if (attempt < 2) {
					await new Promise(r => setTimeout(r, 500));
					if (_shouldAbortContinueSend('setInput-retry-wait#' + (attempt + 1), expectedMode)) return false;
				}
			}
			if (!setOk) {
			...
			// v7.8.2: 触发 trySendMessage 前最后一次短路 — 防止已写入文本但用户关掉开关，仍点了发送
			if (_shouldAbortContinueSend('before-trySend', expectedMode)) {
				// 清空已写入的文本残留（避免下次用户手输时撞车）
				const cleanupEl = findInputEl();
				if (cleanupEl) {
					try { cleanupEl.focus(); document.execCommand('selectAll', false, null); document.execCommand('delete', false, null); } catch {}
				}
			...
			for (let i = 0; i < 5; i++) {
				await new Promise(r => setTimeout(r, 600));
				// v7.8.2: 验证轮询期间也短路（虽然此时发送已触发，但不再继续累计统计/cooldown）
				if (_shouldAbortContinueSend('verify#' + (i + 1), expectedMode)) return false;
				const el = findInputEl();
	...
	// v7.8.5: 修法 A+B
	//   A. bridge 未就绪 = 整个自动切号链路本来就不工作，不再每 120s 反复弹（避免骚扰）
	//      改为：仅在「本会话首次扫到错误时」弹一次，sessionStorage 标记
	//   B. toast 提供「关闭自动切号」按钮，绕过 bridge 直接改本端 settings（同 origin localStorage 可写）
	//      → 关键解药：用户之前死循环（侧栏没开 → bridge 没就绪 → apply-settings 命令推不到补丁）
	...
	function stopBrainlessMode(reason) {
		...
		// v7.8.4: 走 applySettingsChange 触发可观测性日志 + 自动启动 simple 模式
		// 避免手动 settings.continueMode = 'simple' + saveSettings + startSimpleContinue 跳过统一出口
		applySettingsChange({ continueMode: 'simple' }, 'brainless-stop');
		// 通知侧栏更新状态
	...
	async function fireBrainlessContinue() {
		...
		try {
			// v7.8.4: 传 'brainless' — 用户点「强制停止」后 stopBrainlessMode 会切 mode='simple'，此时立即中断避免多发一条
			const sent = await sendContinueMessage(text, 'brainless');
			// v7.8.4: await 后再检查一次——sendContinueMessage 可能在验证轮询中被切模返回 false，此时不应续走 sent 分支
			if (settings.continueMode !== 'brainless') {
				console.log(LOG_PREFIX + '[Brainless] await 后发现模式已切换到 ' + settings.continueMode + '，不更新状态');
				return;
			}
			if (sent) {
				...
				setTimeout(() => {
					// v7.8.4: 3s 内用户可能已停止长任务——闭包内独立短路，避免多余扫描
					if (settings.continueMode !== 'brainless') {
						console.log(LOG_PREFIX + '[Brainless] post-send 3s setTimeout 短路: 模式已切换');
						return;
					}
					try {
	...
	function startBrainlessMode() {
		...
		brainlessTimer = setInterval(() => {
			...
			//   场景：扩展崩溃 / Windsurf 后台进程死掉 / 端口异常 → 用户没法点「强制停止」（force-stop 命令也送不到）
	...
	function _markBridgeUnready(reason) {
		if (_bridgeReady) {
			_bridgeReady = false;
			_bridgeUnreadySince = Date.now();
			console.warn(LOG_PREFIX + '[bridge] ' + reason + '，标记为未就绪');
		}
	}
	function _markBridgeReady(reason) {
		const wasUnready = !_bridgeReady;
		_bridgeReady = true;
		_bridgeUnreadySince = 0;
		if (wasUnready) {
			console.log(LOG_PREFIX + '[bridge] ' + reason);
			// 关键：bridge 从未就绪转就绪时立即回放待同步队列（彻底解决死锁）
			setTimeout(() => { try { flushPendingEnhPatch(); } catch {} }, 300);
		}
	}
	async function bridgePoll() {
		...
		try {
			...
			if (!res.ok) {
				...
				if (_bridgePollFailStreak >= 3) _markBridgeUnready('连续 3 次响应非 200');
				return [];
			}
			_bridgePollFailStreak = 0;
			_markBridgeReady('心跳恢复，重新标记为就绪');
			return await res.json();
		} catch (e) {
			_bridgePollFailStreak++;
			if (_bridgePollFailStreak >= 3) _markBridgeUnready('连续 3 次连接失败');
			return [];
		}
	}
	...
	async function startBridgePolling() {
		...
		// v7.8.5: 走 _markBridgeReady 以触发 flushPendingEnhPatch
		_markBridgeReady('启动后首次就绪');
	...
	function applySettingsChange(newSettings, source) {
		...
		if (changedKeys.length > 0) {
			...
			console.log(LOG_PREFIX + '[apply-settings] 关键开关变更 ' + (source || '') + ': ' + diff);
		}
		// 嵌套对象 guardian.* 单独 diff（用户最常碰的恢复子开关）
		const oldGuardian = old.guardian || {};
		const newGuardian = settings.guardian || {};
		const guardianKeys = ['autoContinueButton', 'autoRetry', 'autoSendOnToolLimit', 'autoApprovePermission', 'dismissCorrupt'];
		const changedGuardian = guardianKeys.filter(k => oldGuardian[k] !== newGuardian[k]);
		if (changedGuardian.length > 0) {
			const diff = changedGuardian.map(k => 'guardian.' + k + ': ' + JSON.stringify(oldGuardian[k]) + ' → ' + JSON.stringify(newGuardian[k])).join(', ');
			console.log(LOG_PREFIX + '[apply-settings] 守护子开关变更 ' + (source || '') + ': ' + diff);
		}
