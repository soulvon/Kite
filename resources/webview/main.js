(function() {
  ...
  function switchAppTab(tab, persist = true) {
    const pages = Array.from(document.querySelectorAll('[data-tab-page], [data-app-tab-page]'));
    const tabs = Array.from(document.querySelectorAll('.app-tab[data-tab], .app-tab[data-app-tab]'));
    if (!pages.length || !tabs.length) return;

    const pageKey = (page) => page.dataset.tabPage || page.dataset.appTabPage || '';
    const tabKey = (btn) => btn.dataset.tab || btn.dataset.appTab || '';
    const activeTab = pages.some(page => pageKey(page) === tab) ? tab : 'account';
    tabs.forEach(btn => {
      const active = tabKey(btn) === activeTab;
      btn.classList.toggle('active', active);
      btn.setAttribute('aria-selected', active ? 'true' : 'false');
    });
    pages.forEach(page => {
      const active = pageKey(page) === activeTab;
    ...
    if (persist) {
      try {
        ...
        st._activeAppTab = activeTab;
        vscode.setState(st);
      } catch {}
      try { localStorage.setItem('ws-pool-active-tab', activeTab); } catch {}
    }
  }
  ...
  function initAppTabs() {
    ...
    tabs.forEach(btn => {
      btn.addEventListener('click', () => switchAppTab(btn.dataset.tab || btn.dataset.appTab || 'account'));
    });

    let saved = '';
    try { saved = (vscode.getState() || {})._activeAppTab || ''; } catch {}
    if (!saved) {
      try { saved = localStorage.getItem('ws-pool-active-tab') || ''; } catch {}
    }
    switchAppTab(saved || 'account', false);
    updateAppTabCounts();
  }

  function initSplitCards() {
    document.querySelectorAll('.split-card').forEach(card => {
      const items = Array.from(card.querySelectorAll('.split-sidebar-item[data-pane]'));
      ...
      const showPane = (pane, scrollToTarget = false) => {
        ...
        const fallbackPane = contents[0]?.dataset.paneContent || contentPane;
        const resolvedPane = hasTargetContent ? contentPane : fallbackPane;
        contents.forEach(content => {
          const active = content.dataset.paneContent === resolvedPane;
        ...
        const tabMap = {
          ...
          'guardian': 'acTabGuardian',
          'long-task': 'acTabLongTask',
        };
        const radioId = tabMap[pane];
        if (radioId) {
  ...
  function switchSubtab(group, target) {
    const bar = document.querySelector('.subtabs-bar[data-subtabs="' + group + '"]');
    if (!bar) return;
    const scope = bar.closest('[data-app-tab-page]') || document;
    const buttons = Array.from(bar.querySelectorAll('.subtab[data-subtab-target]'));
    const pages = Array.from(scope.querySelectorAll('.subtab-content[data-subtab-page]'));
    const activeTarget = pages.some(page => page.dataset.subtabPage === target) ? target : (buttons[0]?.dataset.subtabTarget || '');
    if (!activeTarget) return;

    buttons.forEach(btn => {
      const active = btn.dataset.subtabTarget === activeTarget;
      btn.classList.toggle('active', active);
      btn.setAttribute('aria-selected', active ? 'true' : 'false');
    });
    pages.forEach(page => {
      const active = page.dataset.subtabPage === activeTarget;
      page.classList.toggle('active', active);
      if (active) page.removeAttribute('hidden');
      else page.setAttribute('hidden', '');
    });
  ...
  function initSubtabs() {
    document.querySelectorAll('.subtabs-bar[data-subtabs]').forEach(bar => {
      const group = bar.dataset.subtabs || '';
      const buttons = Array.from(bar.querySelectorAll('.subtab[data-subtab-target]'));
      buttons.forEach(btn => {
        btn.addEventListener('click', () => switchSubtab(group, btn.dataset.subtabTarget || ''));
      });
      const initial = buttons.find(btn => btn.classList.contains('active'))?.dataset.subtabTarget || buttons[0]?.dataset.subtabTarget || '';
      if (initial) switchSubtab(group, initial);
    });
  }
  ...
  // BYOK 面板元素
  const byokStatusPill = $('#byokStatusPill');
  const byokProxyStatus = $('#byokProxyStatus');
  const byokPatchStatus = $('#byokPatchStatus');
  const byokPortStatus = $('#byokPortStatus');
  const byokStatRequests = $('#byokStatRequests');
  const byokStatTokens = $('#byokStatTokens');
  const byokStatRetries = $('#byokStatRetries');
  const byokStatErrors = $('#byokStatErrors');
  const byokStatLastRoute = $('#byokStatLastRoute');
  const byokStatRate = $('#byokStatRate');
  const byokStartBtn = $('#byokStartBtn');
  const byokStopBtn = $('#byokStopBtn');
  const byokRefreshBtn = $('#byokRefreshBtn');
  const byokApplyPatchBtn = $('#byokApplyPatchBtn');
  const byokRestorePatchBtn = $('#byokRestorePatchBtn');
  const byokProviderList = $('#byokProviderList');
  const byokProviderEmpty = $('#byokProviderEmpty');
  const byokProviderModal = $('#byokProviderModal');
  const byokProviderModalTitle = $('#byokProviderModalTitle');
  const byokProviderBackBtn = $('#byokProviderBackBtn');
  const byokProviderCancelBtn = $('#byokProviderCancelBtn');
  const byokProviderSelectedModels = $('#byokProviderSelectedModels');
  const byokProviderSelectedCount = $('#byokProviderSelectedCount');
  const byokProviderModelCatalog = $('#byokProviderModelCatalog');
  const byokProviderModelFilters = $('#byokProviderModelFilters');
  const byokProviderModelSearch = $('#byokProviderModelSearch');
  const byokProviderCustomModel = $('#byokProviderCustomModel');
  const byokProviderAddCustomModelBtn = $('#byokProviderAddCustomModelBtn');
  const byokFetchModelsBtn = $('#byokFetchModelsBtn');
  const byokProviderTestDot = $('#byokProviderTestDot');
  const byokProviderTestText = $('#byokProviderTestText');
  const byokProviderId = $('#byokProviderId');
  const byokProviderName = $('#byokProviderName');
  const byokProviderFormat = $('#byokProviderFormat');
  const byokProviderHost = $('#byokProviderHost');
  const byokProviderPath = $('#byokProviderPath');
  const byokProviderModel = $('#byokProviderModel');
  const byokProviderKey = $('#byokProviderKey');
  const byokProviderEnabled = $('#byokProviderEnabled');
  const byokSaveProviderBtn = $('#byokSaveProviderBtn');
  const byokTestProviderBtn = $('#byokTestProviderBtn');
  const byokDeleteProviderBtn = $('#byokDeleteProviderBtn');
  const byokNewProviderBtn = $('#byokNewProviderBtn');
  const byokProviderMsg = $('#byokProviderMsg');
  const byokNewSlotBtn = $('#byokNewSlotBtn');
  const byokOpenMapSettingsBtn = $('#byokOpenMapSettingsBtn');
  const byokOpenInjectedBtn = $('#byokOpenInjectedBtn');
  const byokModelSearchInput = $('#byokModelSearchInput');
  const byokModelRefreshBtn = $('#byokModelRefreshBtn');
  const byokModelSummary = $('#byokModelSummary');
  const byokSlotEmpty = $('#byokSlotEmpty');
  const byokSlotModal = $('#byokSlotModal');
  const byokSlotModalTitle = $('#byokSlotModalTitle');
  const byokSlotBackBtn = $('#byokSlotBackBtn');
  const byokSlotCancelBtn = $('#byokSlotCancelBtn');
  const byokSlotEditUid = $('#byokSlotEditUid');
  const byokSlotCatalogCount = $('#byokSlotCatalogCount');
  const byokSlotCatalogSearch = $('#byokSlotCatalogSearch');
  const byokSlotCatalog = $('#byokSlotCatalog');
  const byokMappingModelSearch = $('#byokMappingModelSearch');
  const byokMappingModelCatalog = $('#byokMappingModelCatalog');
  const byokSlotEditorStatus = $('#byokSlotEditorStatus');
  const byokModelUid = $('#byokModelUid');
  const byokDisplayName = $('#byokDisplayName');
  const byokSlotProvider = $('#byokSlotProvider');
  const byokSlotModel = $('#byokSlotModel');
  const byokSlotEnabled = $('#byokSlotEnabled');
  const byokSaveSlotBtn = $('#byokSaveSlotBtn');
  const byokDeleteSlotBtn = $('#byokDeleteSlotBtn');
  const byokSlotList = $('#byokSlotList');
  const byokSlotMsg = $('#byokSlotMsg');
  const byokIdeModels = $('#byokIdeModels');
  const byokModelCount = $('#byokModelCount');
  const byokCountProviders = $('#byokCountProviders');
  const byokCountPatches = $('#byokCountPatches');
  const byokLog = $('#byokLog');
  const byokLogCount = $('#byokLogCount');
  const byokLogFilter = $('#byokLogFilter');
  const byokExportLogBtn = $('#byokExportLogBtn');
  const byokClearLogBtn = $('#byokClearLogBtn');
  const byokMapSettingsModal = $('#byokMapSettingsModal');
  const byokMapSettingsBackBtn = $('#byokMapSettingsBackBtn');
  const byokMapSettingsCancelBtn = $('#byokMapSettingsCancelBtn');
  const byokMapSettingsSaveBtn = $('#byokMapSettingsSaveBtn');
  const byokNamePrefix = $('#byokNamePrefix');
  const byokLabelTemplate = $('#byokLabelTemplate');
  const byokLabelPreview = $('#byokLabelPreview');
  const byokMapSettingsStatus = $('#byokMapSettingsStatus');
  const byokFailoverModal = $('#byokFailoverModal');
  const byokFailoverBackBtn = $('#byokFailoverBackBtn');
  const byokFailoverCancelBtn = $('#byokFailoverCancelBtn');
  const byokFailoverSaveBtn = $('#byokFailoverSaveBtn');
  const byokFailoverAddBtn = $('#byokFailoverAddBtn');
  const byokFailoverUid = $('#byokFailoverUid');
  const byokFailoverTitleSlot = $('#byokFailoverTitleSlot');
  const byokFailoverRows = $('#byokFailoverRows');
  const byokFailoverStatus = $('#byokFailoverStatus');
  const byokInjectedModal = $('#byokInjectedModal');
  const byokInjectedBackBtn = $('#byokInjectedBackBtn');
  const byokInjectedCancelBtn = $('#byokInjectedCancelBtn');
  const byokInjectedSaveBtn = $('#byokInjectedSaveBtn');
  const byokInjectedSearch = $('#byokInjectedSearch');
  const byokInjectedFilter = $('#byokInjectedFilter');
  const byokInjectedList = $('#byokInjectedList');
  const byokInjectedCount = $('#byokInjectedCount');
  const byokInjectedStatus = $('#byokInjectedStatus');
  ...
  window.addEventListener('message', (event) => {
    ...
    switch (msg.type) {
      ...
      case 'byokStateSync': {
        if (msg.error) {
          setByokMsg(byokProviderMsg, msg.error, 'err');
        }
        updateByokState(msg.state);
        break;
      }

      case 'byokLog': {
        appendByokLog(msg.log);
        break;
      }

      case 'byokProviderTestResult': {
        const result = msg.result || {};
        handleByokProviderTestResult(msg.providerId, result);
        break;
      }
  ...
  // ==================== BYOK UI ====================
  let byokProviderDraftModels = [];
  let byokProviderCatalogModels = [];
  let byokProviderModelFilter = 'all';
  let byokSelectedSlotUid = '';
  let byokSelectedSlotTarget = null;
  let byokFailoverDraft = [];
  let byokInjectedDraft = [];
  let byokLocalLogs = [];
  let byokHiddenLogLines = new Set();
  let byokRefreshTimer = null;

  const BYOK_COMMON_MODELS = {
    openai: ['gpt-4o', 'gpt-4o-mini', 'o3', 'o3-mini', 'gpt-4.1', 'gpt-4.1-mini', 'deepseek-chat', 'deepseek-reasoner', 'qwen-max'],
    anthropic: ['claude-3-5-sonnet-latest', 'claude-3-7-sonnet-latest', 'claude-sonnet-4-20250514', 'claude-opus-4-20250514']
  };

  function setByokMsg(el, text, kind) {
    if (!el) return;
    el.textContent = text || '';
    el.className = 'byok-msg' + (kind ? ' is-' + kind : '');
  }

  function setByokStatusValue(el, text, kind) {
    if (!el) return;
    el.textContent = text || '-';
    const base = (el === byokProxyStatus || el === byokPatchStatus || el === byokPortStatus) ? 'byok-metric-val' : 'byok-status-value';
    el.className = base + (kind ? ' is-' + kind : '');
    if (el === byokPortStatus) el.className += ' mono';
  }

  function appendByokLog(line) {
    if (!line) return;
    byokLocalLogs = [...byokLocalLogs, line].slice(-200);
    byokHiddenLogLines.delete(line);
    renderByokLogs();
  }
  ...
  function byokNumber(value) {
    const num = Number(value || 0);
    if (!Number.isFinite(num)) return '0';
    return new Intl.NumberFormat().format(num);
  }

  function byokNormalizeTargets(targets) {
    const seen = new Set();
  ...
  function byokProviderById(id) {
    const list = byokState?.providers || [];
    return list.find(p => p.id === id);
  }

  function byokDefaultPath(format) {
    return format === 'anthropic' ? '/v1/messages' : '/v1/chat/completions';
  }

  function byokProviderModels(provider) {
    if (!provider) return [];
    return uniqueStrings([provider.defaultModel, ...(Array.isArray(provider.models) ? provider.models : [])]);
  }

  function byokProviderLogoClass(provider) {
    const lower = `${provider?.name || ''} ${provider?.defaultModel || ''}`.toLowerCase();
    if (lower.includes('anthropic') || lower.includes('claude')) return 'logo-anthropic';
    if (lower.includes('openai') || lower.includes('gpt') || lower.includes('o3')) return 'logo-openai';
    if (lower.includes('deepseek')) return 'logo-deepseek';
    if (lower.includes('gemini') || lower.includes('google')) return 'logo-gemini';
    if (lower.includes('qwen') || lower.includes('dashscope') || lower.includes('通义')) return 'logo-qwen';
    if (lower.includes('xai') || lower.includes('grok')) return 'logo-xai';
    return provider?.apiFormat === 'anthropic' ? 'logo-anthropic' : 'logo-openai';
  }

  function byokInitial(name) {
    return String(name || 'P').trim().charAt(0).toUpperCase() || 'P';
  }

  function byokModelMatchesFilter(model, filter) {
    const s = String(model || '').toLowerCase();
    if (!filter || filter === 'all') return true;
    if (filter === 'reason') return /(reason|thinking|o1|o3|r1|deepseek-reasoner|qwq)/i.test(s);
    if (filter === 'vision') return /(vision|gpt-4o|claude|gemini|qwen-vl|vl|pixtral|llava)/i.test(s);
    if (filter === 'free') return /(:free|-free|free\b)/i.test(s);
    if (filter === 'embedding') return /(embed|embedding|text-embedding|bge|rerank)/i.test(s);
    return true;
  }

  function byokModelIconKey(model) {
    const s = String(model || '').toLowerCase();
    if (s.includes('claude')) return 'A';
    if (s.includes('gpt') || /^o\d/.test(s)) return 'O';
    if (s.includes('deepseek')) return 'D';
    if (s.includes('gemini')) return 'G';
    if (s.includes('qwen')) return 'Q';
    if (s.includes('grok') || s.includes('xai')) return 'X';
    return 'M';
  }

  function setByokModalOpen(modal, open) {
    if (!modal) return;
    if (open) {
      modal.hidden = false;
  ...
  function syncProviderDefaultModel() {
    if (byokProviderModel) byokProviderModel.value = byokProviderDraftModels[0] || '';
    if (byokProviderSelectedCount) byokProviderSelectedCount.textContent = String(byokProviderDraftModels.length);
  }

  function ensureProviderCatalog(models) {
    const format = byokProviderFormat?.value || 'openai';
    byokProviderCatalogModels = uniqueStrings([
      ...byokProviderCatalogModels,
      ...byokProviderDraftModels,
      ...(models || []),
      ...(BYOK_COMMON_MODELS[format] || [])
    ]);
  }

  function renderProviderSelectedModels() {
    syncProviderDefaultModel();
    if (!byokProviderSelectedModels) return;
    if (!byokProviderDraftModels.length) {
      byokProviderSelectedModels.innerHTML = '<div class="byok-row-meta">尚未选择模型</div>';
      return;
    }
    byokProviderSelectedModels.innerHTML = byokProviderDraftModels.map((model, idx) => `
      <div class="byok-selected-model-row" data-provider-model="${escAttr(model)}">
        <div class="byok-catalog-main">
          <div class="byok-catalog-icon">${escHtml(byokModelIconKey(model))}</div>
          <div class="byok-catalog-text">
            <div class="byok-catalog-title">${escHtml(model)}</div>
            <div class="byok-catalog-meta">${idx === 0 ? '默认模型' : '可用模型'}</div>
          </div>
        </div>
        <div class="byok-inline-actions">
          ${idx === 0 ? '<span class="byok-row-badge is-on">默认</span>' : `<button class="byok-mini-btn" type="button" data-provider-model-action="default">设默认</button>`}
          <button class="byok-mini-btn is-danger" type="button" data-provider-model-action="remove">移除</button>
        </div>
      </div>
    `).join('');
  }
  ...
  function renderProviderModelCatalog() {
    if (!byokProviderModelCatalog) return;
    ensureProviderCatalog();
    const query = (byokProviderModelSearch?.value || '').toLowerCase().trim();
    const filtered = byokProviderCatalogModels
      .filter(model => byokModelMatchesFilter(model, byokProviderModelFilter))
      .filter(model => !query || model.toLowerCase().includes(query));
    if (!filtered.length) {
      byokProviderModelCatalog.innerHTML = '<div class="byok-empty">暂无匹配模型，可在下方手动添加。</div>';
      return;
    }
    const selected = new Set(byokProviderDraftModels);
    byokProviderModelCatalog.innerHTML = filtered.map(model => {
      const isSelected = selected.has(model);
      return `
        <div class="byok-catalog-item${isSelected ? ' selected' : ''}" data-provider-catalog-model="${escAttr(model)}">
          <div class="byok-catalog-main">
            <div class="byok-catalog-icon">${escHtml(byokModelIconKey(model))}</div>
            <div class="byok-catalog-text">
              <div class="byok-catalog-title">${escHtml(model)}</div>
              <div class="byok-catalog-meta">${isSelected ? '已加入供应商模型列表' : '点击加入模型列表'}</div>
            </div>
          </div>
          <div class="byok-catalog-tags">
            ${isSelected ? '<span class="byok-row-badge is-on">已选</span>' : '<span class="byok-row-badge">可选</span>'}
          </div>
        </div>
      `;
    }).join('');
  ...
  function addProviderDraftModel(model, makeDefault = false) {
    const clean = String(model || '').trim();
    if (!clean) return;
    byokProviderDraftModels = byokProviderDraftModels.filter(x => x !== clean);
    if (makeDefault || !byokProviderDraftModels.length) byokProviderDraftModels.unshift(clean);
    else byokProviderDraftModels.push(clean);
    ensureProviderCatalog([clean]);
    renderProviderSelectedModels();
    renderProviderModelCatalog();
  }

  function removeProviderDraftModel(model) {
    byokProviderDraftModels = byokProviderDraftModels.filter(x => x !== model);
    renderProviderSelectedModels();
    renderProviderModelCatalog();
  }

  function setProviderDraftDefault(model) {
    addProviderDraftModel(model, true);
  }

  function openByokProviderEditor(id) {
    const provider = id ? byokProviderById(id) : null;
    byokSelectedProviderId = provider?.id || '';
    if (byokProviderId) byokProviderId.value = provider?.id || '';
    if (byokProviderName) byokProviderName.value = provider?.name || '';
    if (byokProviderFormat) byokProviderFormat.value = provider?.apiFormat || 'openai';
    if (byokProviderHost) byokProviderHost.value = provider?.apiHost || '';
    if (byokProviderPath) byokProviderPath.value = provider?.apiPath || byokDefaultPath(provider?.apiFormat || 'openai');
    if (byokProviderKey) byokProviderKey.value = '';
    if (byokProviderEnabled) byokProviderEnabled.checked = provider ? provider.enabled !== false : true;
    if (byokProviderModalTitle) byokProviderModalTitle.textContent = provider ? '编辑供应商' : '新增供应商';
    if (byokDeleteProviderBtn) byokDeleteProviderBtn.hidden = !provider;
    if (byokProviderTestDot) byokProviderTestDot.className = 'conn-dot no';
    if (byokProviderTestText) byokProviderTestText.textContent = provider?.hasApiKey ? '已保存密钥：' + (provider.maskedApiKey || '***') : '未测试';
    byokProviderDraftModels = byokProviderModels(provider);
    byokProviderCatalogModels = uniqueStrings([...byokProviderDraftModels, ...(BYOK_COMMON_MODELS[byokProviderFormat?.value || 'openai'] || [])]);
    byokProviderModelFilter = 'all';
    if (byokProviderModelSearch) byokProviderModelSearch.value = '';
    if (byokProviderCustomModel) byokProviderCustomModel.value = '';
    document.querySelectorAll('#byokProviderModelFilters button').forEach(btn => {
      btn.classList.toggle('active', btn.dataset.filter === 'all');
    });
    setByokMsg(byokProviderMsg, provider?.hasApiKey ? '已加载供应商配置' : '', provider?.hasApiKey ? 'ok' : '');
    renderProviderSelectedModels();
    renderProviderModelCatalog();
    renderByokProviders();
    setByokModalOpen(byokProviderModal, true);
    setTimeout(() => byokProviderName?.focus(), 30);
  }

  function closeByokProviderEditor() {
    setByokModalOpen(byokProviderModal, false);
  }

  function collectByokProviderForm() {
    const models = uniqueStrings(byokProviderDraftModels);
    return {
      id: byokProviderId?.value || undefined,
      name: byokProviderName?.value || '',
      apiFormat: byokProviderFormat?.value || 'openai',
      apiHost: byokProviderHost?.value || '',
      apiPath: byokProviderPath?.value || '',
      defaultModel: models[0] || byokProviderModel?.value || '',
      models,
      apiKey: byokProviderKey?.value || '',
      enabled: byokProviderEnabled ? byokProviderEnabled.checked : true,
    };
  }

  function saveByokProviderFromEditor() {
    const provider = collectByokProviderForm();
    if (!provider.name.trim()) {
      setByokMsg(byokProviderMsg, '请填写供应商名称', 'err');
      byokProviderName?.focus();
      return;
    }
    if (!provider.apiHost.trim()) {
      setByokMsg(byokProviderMsg, '请填写 API Base URL', 'err');
      byokProviderHost?.focus();
      return;
    }
    if (!provider.defaultModel.trim()) {
      setByokMsg(byokProviderMsg, '请至少添加一个模型', 'err');
      byokProviderCustomModel?.focus();
      return;
    }
    setByokMsg(byokProviderMsg, '保存中...', '');
    postMsg('byokSaveProvider', { provider });
    closeByokProviderEditor();
    showToast('供应商保存已提交', 'success', 1800);
  }

  function testByokProvider(id, apiKeyOverride = '') {
    const providerId = id || byokProviderId?.value || byokSelectedProviderId;
    if (!providerId) {
      setByokMsg(byokProviderMsg, '首次添加请先保存供应商，再测试读取模型', 'err');
      return;
    }
    if (byokProviderTestDot) byokProviderTestDot.className = 'conn-dot no';
    if (byokProviderTestText) byokProviderTestText.textContent = '测试中...';
    setByokMsg(byokProviderMsg, '测试连接中...', '');
    postMsg('byokTestProvider', { providerId, apiKey: apiKeyOverride || byokProviderKey?.value || '' });
  }

  function handleByokProviderTestResult(providerId, result) {
    const ok = !!result.ok;
    setByokMsg(byokProviderMsg, result.message || (ok ? '测试成功' : '测试失败'), ok ? 'ok' : 'err');
    if (byokProviderTestDot) byokProviderTestDot.className = 'conn-dot ' + (ok ? 'ok' : 'no');
    if (byokProviderTestText) byokProviderTestText.textContent = result.message || (ok ? '测试成功' : '测试失败');
    const editorOpen = !!(byokProviderModal && !byokProviderModal.hidden);
    const sameProvider = !providerId || providerId === (byokProviderId?.value || byokSelectedProviderId);
    if (editorOpen && sameProvider && Array.isArray(result.models) && result.models.length) {
      ensureProviderCatalog(result.models);
      if (!byokProviderDraftModels.length) {
        byokProviderDraftModels = uniqueStrings(result.models).slice(0, 12);
      }
      renderProviderSelectedModels();
      renderProviderModelCatalog();
      showToast('已读取模型列表', 'success', 1800);
    } else if (Array.isArray(result.models) && result.models.length) {
      showToast(`测试成功，读取到 ${result.models.length} 个模型`, 'success', 1800);
    }
    if (providerId) {
      renderByokProviders();
    }
  ...
  function renderByokProviders() {
    if (!byokProviderList) return;
    const list = byokState?.providers || [];
    if (byokCountProviders) byokCountProviders.textContent = String(list.length);
    if (byokProviderEmpty) byokProviderEmpty.hidden = !!list.length;
    if (!list.length) {
      byokProviderList.innerHTML = '';
      return;
    }
    byokProviderList.innerHTML = list.map(provider => {
      const enabled = provider.enabled !== false;
      const models = byokProviderModels(provider);
      const modelBadges = (models.length ? models : ['未选择']).slice(0, 8).map((model, idx) => `
        <span class="byok-model-chip${idx === 0 && models.length ? ' is-default' : ''}" title="${escAttr(model)}">
          <span>${escHtml(model)}</span>${idx === 0 && models.length ? '<small>默认</small>' : ''}
        </span>
      `).join('');
      const overflow = models.length > 8 ? `<span class="byok-summary-chip">+${models.length - 8}</span>` : '';
      return `
        <div class="byok-provider-card${provider.id === byokSelectedProviderId ? ' is-active' : ''}" data-provider-id="${escAttr(provider.id)}">
          <div class="byok-provider-head">
            <div class="byok-provider-logo ${byokProviderLogoClass(provider)}">${escHtml(byokInitial(provider.name || provider.id))}</div>
            <div class="byok-provider-info">
              <div class="byok-provider-name">${escHtml(provider.name || provider.id)}</div>
              <div class="byok-provider-meta">${escHtml(provider.apiFormat || 'openai')} · ${escHtml(provider.apiPath || byokDefaultPath(provider.apiFormat))}</div>
            </div>
            <span class="byok-row-badge${enabled ? ' is-on' : ''}">${enabled ? '启用' : '停用'}</span>
          </div>
          <div class="byok-provider-body">
            <div class="byok-card-label">已选模型</div>
            <div class="byok-model-stack">${modelBadges}${overflow}</div>
            <div class="byok-card-label">${provider.hasApiKey ? 'Key 已保存' : '未保存 Key'} · ${escHtml(provider.maskedApiKey || provider.apiHost || '-')}</div>
          </div>
          <div class="byok-provider-footer">
            <span class="byok-row-badge${provider.hasApiKey ? ' is-on' : ''}">${provider.hasApiKey ? '可测试' : '缺 Key'}</span>
            <div class="byok-inline-actions">
              <button class="byok-mini-btn" type="button" data-provider-action="test" data-provider-id="${escAttr(provider.id)}">测试</button>
              <button class="byok-mini-btn" type="button" data-provider-action="toggle" data-provider-id="${escAttr(provider.id)}">${enabled ? '停用' : '启用'}</button>
              <button class="byok-mini-btn" type="button" data-provider-action="edit" data-provider-id="${escAttr(provider.id)}">编辑</button>
              <button class="byok-mini-btn is-danger" type="button" data-provider-action="delete" data-provider-id="${escAttr(provider.id)}">删除</button>
            </div>
          </div>
        </div>
      `;
  ...
  function getByokIdeModelList() {
    const builtin = [
      { modelUid: 'MODEL_XAI_GROK_3', label: 'xAI Grok-3' },
      { modelUid: 'MODEL_XAI_GROK_3_MINI_REASONING', label: 'xAI Grok-3 mini Thinking' },
      { modelUid: 'MODEL_PRIVATE_4', label: 'Grok Code Fast 1' },
    ];
    const fromState = Array.isArray(byokState?.ideModels) ? byokState.ideModels : [];
    const injected = Array.isArray(byokState?.modelMap?.injected) ? byokState.modelMap.injected : [];
    const seen = new Set();
    return [...fromState, ...injected, ...builtin].filter(model => {
      const uid = model?.modelUid || model?.id;
      if (!uid || seen.has(uid)) return false;
      seen.add(uid);
      return true;
    }).map(model => ({
      modelUid: model.modelUid || model.id,
      label: model.label || model.name || model.modelUid || model.id,
      supportsImages: model.supportsImages
    }));
  }

  function renderByokIdeModels() {
    if (!byokIdeModels) return;
    const models = getByokIdeModelList();
    byokIdeModels.innerHTML = models.map(m => `<option value="${escAttr(m.modelUid)}">${escHtml(m.label || m.modelUid)}</option>`).join('');
  }

  function getAllByokProviderModels() {
    const out = [];
    (byokState?.providers || []).forEach(provider => {
      if (provider.enabled === false) return;
      byokProviderModels(provider).forEach(model => {
        out.push({ providerId: provider.id, providerName: provider.name || provider.id, model });
      });
    });
    return out;
  }

  function renderByokSlotProviderOptions() {
    if (!byokSlotProvider) return;
    const providers = byokState?.providers || [];
    byokSlotProvider.innerHTML = providers.length
      ? providers.map(p => `<option value="${escAttr(p.id)}">${escHtml(p.name || p.id)}</option>`).join('')
      : '<option value="">先添加供应商</option>';
  }

  function slotDisplayName(uid) {
    const match = getByokIdeModelList().find(m => m.modelUid === uid);
    return match?.label || uid;
  }

  function byokFindSlot(uid) {
    return (byokState?.modelMap?.slots || []).find(slot => slot.modelUid === uid);
  }

  function byokTargetText(target) {
    if (!target || !target.providerId || !target.model) return '-';
    const provider = byokProviderById(target.providerId);
    return `${provider?.name || target.providerId} / ${target.model}`;
  }

  function renderByokTemplatePreview() {
    if (!byokLabelPreview) return;
    const template = byokLabelTemplate?.value || '{prefix} {label} ({provider})';
    const vars = {
      prefix: byokNamePrefix?.value || 'BYOK',
      label: 'Claude Sonnet',
      provider: 'OpenRouter',
      apiModel: 'anthropic/claude-sonnet-4'
    };
    let text = template || '{prefix} {label} ({provider})';
    Object.keys(vars).forEach(key => {
      text = text.replace(new RegExp('\\{' + key + '\\}', 'g'), vars[key]);
    });
    byokLabelPreview.textContent = text.replace(/[ \t]{2,}/g, ' ').trim() || 'Claude Sonnet';
  }

  function openByokMapSettings() {
    const map = byokState?.modelMap || {};
    if (byokNamePrefix) byokNamePrefix.value = map.namePrefix || 'BYOK';
    if (byokLabelTemplate) byokLabelTemplate.value = map.labelTemplate || '{prefix} {label} ({provider})';
    if (byokMapSettingsStatus) byokMapSettingsStatus.textContent = '保存后会在下次刷新模型目录时生效';
    renderByokTemplatePreview();
    setByokModalOpen(byokMapSettingsModal, true);
    setTimeout(() => byokNamePrefix?.focus(), 30);
  }

  function closeByokMapSettings() {
    setByokModalOpen(byokMapSettingsModal, false);
  }

  function saveByokMapSettings() {
    const settings = {
      namePrefix: byokNamePrefix?.value || '',
      labelTemplate: byokLabelTemplate?.value || '{prefix} {label} ({provider})'
    };
    if (byokMapSettingsStatus) byokMapSettingsStatus.textContent = '保存中...';
    postMsg('byokSaveModelMapSettings', { settings });
    closeByokMapSettings();
    showToast('显示名设置保存已提交', 'success', 1800);
  }

  function failoverProviderOptions(selectedId) {
    const providers = byokState?.providers || [];
    if (!providers.length) return '<option value="">先添加供应商</option>';
    return providers.map(provider => `
      <option value="${escAttr(provider.id)}"${provider.id === selectedId ? ' selected' : ''}>
        ${escHtml(provider.name || provider.id)}
      </option>
    `).join('');
  }

  function failoverModelOptions(providerId, selectedModel) {
    const provider = byokProviderById(providerId) || (byokState?.providers || [])[0];
    const models = byokProviderModels(provider);
    const options = uniqueStrings([selectedModel, ...models]);
    if (!options.length) return '<option value="">先在供应商中添加模型</option>';
    return options.map(model => `<option value="${escAttr(model)}"${model === selectedModel ? ' selected' : ''}>${escHtml(model)}</option>`).join('');
  }

  function renderFailoverRows() {
    if (!byokFailoverRows) return;
    if (!byokFailoverDraft.length) {
      byokFailoverRows.innerHTML = '<div class="byok-empty">还没有目标模型，点击「添加备用」选择一路。</div>';
      return;
    }
    byokFailoverRows.innerHTML = byokFailoverDraft.map((target, idx) => `
      <div class="byok-failover-row" data-failover-index="${idx}">
        <div class="byok-row-index">${idx === 0 ? '主' : idx + 1}</div>
        <select class="byok-input" data-failover-field="providerId">
          ${failoverProviderOptions(target.providerId)}
        </select>
        <select class="byok-input" data-failover-field="model">
          ${failoverModelOptions(target.providerId, target.model)}
        </select>
        <div class="byok-inline-actions">
          <button class="byok-mini-btn" type="button" data-failover-action="up" ${idx === 0 ? 'disabled' : ''}>上移</button>
          <button class="byok-mini-btn" type="button" data-failover-action="down" ${idx === byokFailoverDraft.length - 1 ? 'disabled' : ''}>下移</button>
          <button class="byok-mini-btn is-danger" type="button" data-failover-action="remove">移除</button>
        </div>
      </div>
    `).join('');
    if (byokFailoverStatus) {
      const backups = Math.max(0, byokFailoverDraft.length - 1);
      byokFailoverStatus.textContent = backups ? `主路由 + ${backups} 个备用目标` : '当前只有主路由';
    }
  }
  ...
  function addFailoverRow() {
    ...
    if (!target) {
      if (byokFailoverStatus) byokFailoverStatus.textContent = '请先添加供应商和模型';
      return;
    }
    byokFailoverDraft.push({ providerId: target.providerId, model: target.model });
    renderFailoverRows();
  }

  function openByokFailoverEditor(uid) {
    const slot = byokFindSlot(uid);
    if (!slot) return;
    byokFailoverDraft = byokNormalizeTargets(slot.targets);
    if (!byokFailoverDraft.length) {
      const target = chooseDefaultTarget();
      if (target) byokFailoverDraft = [{ providerId: target.providerId, model: target.model }];
    }
    if (byokFailoverUid) byokFailoverUid.value = slot.modelUid;
    if (byokFailoverTitleSlot) byokFailoverTitleSlot.textContent = slot.displayName || slotDisplayName(slot.modelUid);
    renderFailoverRows();
    setByokModalOpen(byokFailoverModal, true);
  }

  function closeByokFailoverEditor() {
    setByokModalOpen(byokFailoverModal, false);
  }

  function saveByokFailover() {
    const uid = byokFailoverUid?.value || '';
    const slot = byokFindSlot(uid);
    const targets = byokNormalizeTargets(byokFailoverDraft);
    if (!slot || !targets.length) {
      if (byokFailoverStatus) byokFailoverStatus.textContent = '至少保留一个有效目标模型';
      return;
    }
    postMsg('byokSaveSlot', { slot: { ...slot, targets } });
    closeByokFailoverEditor();
    showToast('故障转移链保存已提交', 'success', 1800);
  }

  function getByokCatalogList() {
    const catalog = Array.isArray(byokState?.catalog) ? byokState.catalog : [];
    const injected = Array.isArray(byokState?.modelMap?.injected) ? byokState.modelMap.injected : [];
    const captured = Array.isArray(byokState?.ideModels) ? byokState.ideModels : [];
    const seen = new Set();
    return [...injected, ...captured, ...catalog].filter(item => {
      const uid = item?.modelUid || item?.id;
      if (!uid || seen.has(uid)) return false;
      seen.add(uid);
      return true;
    }).map(item => ({
      modelUid: item.modelUid || item.id,
      label: item.label || item.name || item.modelUid || item.id,
      apiId: item.apiId || item.model || '',
      contextWindow: item.contextWindow,
      supportsImages: item.supportsImages !== false,
      noApiIdHint: item.noApiIdHint || ''
    }));
  }

  function injectedDraftByUid(uid) {
    return byokInjectedDraft.find(item => item.modelUid === uid);
  }

  function setInjectedDraftEnabled(model, enabled) {
    byokInjectedDraft = byokInjectedDraft.filter(item => item.modelUid !== model.modelUid);
    if (enabled) {
      const provider = (byokState?.providers || []).find(item => item.enabled !== false) || (byokState?.providers || [])[0];
      byokInjectedDraft.push({
        modelUid: model.modelUid,
        label: model.label || model.modelUid,
        providerId: provider?.id || '',
        model: byokProviderModels(provider)[0] || model.apiId || '',
  ...
  function renderInjectedEditor() {
    if (!byokInjectedList) return;
    const query = (byokInjectedSearch?.value || '').toLowerCase().trim();
    const filter = byokInjectedFilter?.value || 'all';
    const catalog = getByokCatalogList();
    const rows = catalog.filter(model => {
      const draft = injectedDraftByUid(model.modelUid);
      const hay = `${model.label || ''} ${model.modelUid || ''} ${model.apiId || ''}`.toLowerCase();
      if (query && !hay.includes(query)) return false;
      if (filter === 'enabled') return !!draft;
      if (filter === 'vision') return model.supportsImages !== false;
      if (filter === 'missing') return !!draft && (!draft.providerId || !draft.model);
      return true;
    });
    if (byokInjectedCount) byokInjectedCount.textContent = String(byokInjectedDraft.length);
    if (!rows.length) {
      byokInjectedList.innerHTML = '<div class="byok-empty">暂无匹配槽位。</div>';
      return;
    }
    const providers = byokState?.providers || [];
    byokInjectedList.innerHTML = rows.map(model => {
      const draft = injectedDraftByUid(model.modelUid);
      const providerOptions = ['<option value="">选择供应商</option>', ...providers.map(provider => `
        <option value="${escAttr(provider.id)}"${draft?.providerId === provider.id ? ' selected' : ''}>${escHtml(provider.name || provider.id)}</option>
      `)].join('');
      const provider = draft?.providerId ? byokProviderById(draft.providerId) : null;
      const modelOptions = uniqueStrings([draft?.model, model.apiId, ...byokProviderModels(provider)]).filter(Boolean)
        .map(item => `<option value="${escAttr(item)}"${draft?.model === item ? ' selected' : ''}>${escHtml(item)}</option>`)
        .join('');
      const enabled = !!draft;
      return `
        <div class="byok-injected-row${enabled ? ' is-enabled' : ''}" data-injected-uid="${escAttr(model.modelUid)}">
          <label class="byok-check byok-injected-toggle">
            <input type="checkbox" data-injected-action="toggle" ${enabled ? 'checked' : ''}>
            <span>${enabled ? '启用' : '未启用'}</span>
          </label>
          <div class="byok-injected-main">
            <div class="byok-row-title">${escHtml(model.label || model.modelUid)}</div>
            <div class="byok-row-meta">${escHtml(model.modelUid)}${model.apiId ? ' · ' + escHtml(model.apiId) : ''}</div>
          </div>
          <select class="byok-input" data-injected-field="providerId" ${enabled ? '' : 'disabled'}>${providerOptions}</select>
          <select class="byok-input" data-injected-field="model" ${enabled ? '' : 'disabled'}>
            ${modelOptions || '<option value="">先选择供应商</option>'}
          </select>
          <label class="byok-check byok-injected-vision">
            <input type="checkbox" data-injected-field="supportsImages" ${draft?.supportsImages !== false && model.supportsImages !== false ? 'checked' : ''} ${enabled ? '' : 'disabled'}>
            图片
          </label>
        </div>
      `;
    }).join('');
    if (byokInjectedStatus) byokInjectedStatus.textContent = `${byokInjectedDraft.length} 个槽位会被注入模型目录`;
  }

  function openByokInjectedEditor() {
    byokInjectedDraft = JSON.parse(JSON.stringify(byokState?.modelMap?.injected || []));
    if (byokInjectedSearch) byokInjectedSearch.value = '';
    if (byokInjectedFilter) byokInjectedFilter.value = 'all';
    renderInjectedEditor();
    setByokModalOpen(byokInjectedModal, true);
  }

  function closeByokInjectedEditor() {
    setByokModalOpen(byokInjectedModal, false);
  }

  function saveByokInjectedEditor() {
    postMsg('byokSaveInjected', { injected: byokInjectedDraft });
    closeByokInjectedEditor();
    showToast('扩展槽位保存已提交', 'success', 1800);
  }

  function updateSlotHiddenFields() {
    if (byokModelUid) byokModelUid.value = byokSelectedSlotUid || '';
    if (byokSlotProvider) byokSlotProvider.value = byokSelectedSlotTarget?.providerId || '';
    if (byokSlotModel) byokSlotModel.value = byokSelectedSlotTarget?.model || '';
    const targetText = byokSelectedSlotTarget
      ? `${byokSelectedSlotTarget.providerName || byokSelectedSlotTarget.providerId} / ${byokSelectedSlotTarget.model}`
      : '未选择目标模型';
    if (byokSlotEditorStatus) byokSlotEditorStatus.textContent = byokSelectedSlotUid ? `${slotDisplayName(byokSelectedSlotUid)} → ${targetText}` : '请选择槽位和目标模型';
  }

  function renderSlotCatalog() {
    if (!byokSlotCatalog) return;
    const models = getByokIdeModelList();
    const editingUid = byokSlotEditUid?.value || '';
    const used = new Set((byokState?.modelMap?.slots || []).map(slot => slot.modelUid));
    if (editingUid) used.delete(editingUid);
    const query = (byokSlotCatalogSearch?.value || '').toLowerCase().trim();
    const filtered = models.filter(model => {
      const hay = `${model.label || ''} ${model.modelUid || ''}`.toLowerCase();
      return !query || hay.includes(query);
    });
    const availableCount = filtered.filter(model => !used.has(model.modelUid)).length;
    if (byokSlotCatalogCount) byokSlotCatalogCount.textContent = `${availableCount} 可用 / ${filtered.length} 显示`;
    if (!filtered.length) {
      byokSlotCatalog.innerHTML = '<div class="byok-empty">暂无匹配槽位。启动 Sidecar 后会自动捕获 IDE 模型列表。</div>';
      return;
    }
    byokSlotCatalog.innerHTML = filtered.map(model => {
      const taken = used.has(model.modelUid);
      const selected = byokSelectedSlotUid === model.modelUid;
      return `
        <div class="byok-catalog-item${selected ? ' selected' : ''}${taken ? ' disabled' : ''}" data-slot-uid="${escAttr(model.modelUid)}">
          <div class="byok-catalog-main">
            <div class="byok-catalog-icon">${selected ? '✓' : 'S'}</div>
            <div class="byok-catalog-text">
              <div class="byok-catalog-title">${escHtml(model.label || model.modelUid)}</div>
              <div class="byok-catalog-meta">${escHtml(model.modelUid)}</div>
            </div>
          </div>
          <div class="byok-catalog-tags">
            ${taken ? '<span class="byok-row-badge">已绑定</span>' : ''}
            ${selected ? '<span class="byok-row-badge is-on">已选</span>' : ''}
          </div>
        </div>
      `;
    }).join('');
  ...
  function renderMappingModelCatalog() {
    if (!byokMappingModelCatalog) return;
    const query = (byokMappingModelSearch?.value || '').toLowerCase().trim();
    const models = getAllByokProviderModels().filter(item => {
      const hay = `${item.providerName || ''} ${item.providerId || ''} ${item.model || ''}`.toLowerCase();
      return !query || hay.includes(query);
    });
    if (!models.length) {
      byokMappingModelCatalog.innerHTML = '<div class="byok-empty">暂无可选目标模型，请先在供应商里添加模型并启用供应商。</div>';
      return;
    }
    byokMappingModelCatalog.innerHTML = models.map(item => {
      const selected = byokSelectedSlotTarget?.providerId === item.providerId && byokSelectedSlotTarget?.model === item.model;
      return `
        <div class="byok-catalog-item${selected ? ' selected' : ''}" data-target-provider="${escAttr(item.providerId)}" data-target-model="${escAttr(item.model)}">
          <div class="byok-catalog-main">
            <div class="byok-catalog-icon ${byokProviderLogoClass(byokProviderById(item.providerId))}">${escHtml(byokInitial(item.providerName))}</div>
            <div class="byok-catalog-text">
              <div class="byok-catalog-title">${escHtml(item.model)}</div>
              <div class="byok-catalog-meta">${escHtml(item.providerName || item.providerId)}</div>
            </div>
          </div>
          <div class="byok-catalog-tags">
            ${selected ? '<span class="byok-row-badge is-on">已选</span>' : '<span class="byok-row-badge">可选</span>'}
          </div>
        </div>
      `;
    }).join('');
  ...
  function chooseDefaultTarget() {
    return getAllByokProviderModels()[0] || null;
  }

  function openByokSlotEditor(uid) {
    const slots = byokState?.modelMap?.slots || [];
    const editing = uid ? slots.find(slot => slot.modelUid === uid) : null;
    const target = (editing?.targets || [])[0];
    const provider = target ? byokProviderById(target.providerId) : null;
    byokSelectedSlotUid = editing?.modelUid || '';
    if (!byokSelectedSlotUid) {
      const used = new Set(slots.map(slot => slot.modelUid));
      byokSelectedSlotUid = getByokIdeModelList().find(model => !used.has(model.modelUid))?.modelUid || '';
    }
    byokSelectedSlotTarget = target
      ? { providerId: target.providerId, providerName: provider?.name || target.providerId, model: target.model }
      : chooseDefaultTarget();
    if (byokSlotEditUid) byokSlotEditUid.value = editing?.modelUid || '';
    if (byokDisplayName) byokDisplayName.value = editing?.displayName || '';
    if (byokSlotEnabled) byokSlotEnabled.checked = editing ? editing.enabled !== false : true;
    if (byokSlotModalTitle) byokSlotModalTitle.textContent = editing ? '编辑映射' : '添加映射';
    if (byokDeleteSlotBtn) byokDeleteSlotBtn.hidden = !editing;
    if (byokSlotCatalogSearch) byokSlotCatalogSearch.value = '';
    if (byokMappingModelSearch) byokMappingModelSearch.value = '';
    updateSlotHiddenFields();
    renderSlotCatalog();
    renderMappingModelCatalog();
    setByokMsg(byokSlotMsg, '', '');
    setByokModalOpen(byokSlotModal, true);
  }

  function closeByokSlotEditor() {
    setByokModalOpen(byokSlotModal, false);
  }

  function renderByokModelSummary(slots) {
    if (!byokModelSummary) return;
    const enabled = slots.filter(slot => slot.enabled !== false && (slot.targets || []).length).length;
    const missing = slots.length - enabled;
    const backups = slots.reduce((sum, slot) => sum + Math.max(0, (slot.targets || []).length - 1), 0);
    const injected = Array.isArray(byokState?.modelMap?.injected) ? byokState.modelMap.injected.length : 0;
    const providers = new Set();
    slots.forEach(slot => (slot.targets || []).forEach(target => target.providerId && providers.add(target.providerId)));
    byokModelSummary.innerHTML = [
      `<span class="byok-summary-chip">映射 <b>${slots.length}</b></span>`,
      `<span class="byok-summary-chip">启用 <b>${enabled}</b></span>`,
      `<span class="byok-summary-chip">供应商 <b>${providers.size}</b></span>`,
      backups > 0 ? `<span class="byok-summary-chip">备用 <b>${backups}</b></span>` : '',
      injected > 0 ? `<span class="byok-summary-chip">扩展 <b>${injected}</b></span>` : '',
      missing > 0 ? `<span class="byok-summary-chip">待配置 <b>${missing}</b></span>` : ''
    ].filter(Boolean).join('');
  }

  function renderByokSlots() {
    if (!byokSlotList) return;
    const slots = byokState?.modelMap?.slots || [];
    const query = (byokModelSearchInput?.value || '').toLowerCase().trim();
    const filtered = slots.filter(slot => {
      const targetText = (slot.targets || []).map(target => {
        const provider = byokProviderById(target.providerId);
        return `${provider?.name || ''} ${target.providerId || ''} ${target.model || ''}`;
      }).join(' ');
      const hay = `${slot.displayName || ''} ${slot.modelUid || ''} ${slotDisplayName(slot.modelUid)} ${targetText}`.toLowerCase();
      return !query || hay.includes(query);
    });
    if (byokModelCount) byokModelCount.textContent = String(slots.length);
    if (byokSlotEmpty) byokSlotEmpty.hidden = !!slots.length;
    renderByokModelSummary(slots);
    if (!filtered.length) {
      byokSlotList.innerHTML = slots.length ? '<div class="byok-empty">没有匹配的映射。</div>' : '';
      return;
    }
    byokSlotList.innerHTML = filtered.map(slot => {
      const targets = byokNormalizeTargets(slot.targets);
      const target = targets[0] || {};
      const provider = byokProviderById(target.providerId);
      const enabled = slot.enabled !== false && target.providerId && target.model;
      const backupCount = Math.max(0, targets.length - 1);
      return `
        <div class="byok-slot-row" data-model-uid="${escAttr(slot.modelUid)}">
          <div class="byok-slot-main">
            <div>
              <div class="byok-row-title">${escHtml(slot.displayName || slotDisplayName(slot.modelUid))}</div>
              <div class="byok-row-meta">${escHtml(slot.modelUid)}</div>
            </div>
            <span class="byok-row-badge${enabled ? ' is-on' : ''}">${enabled ? '启用' : '未配置'}</span>
          </div>
          <div class="byok-slot-route">
            <span>${escHtml(provider?.name || target.providerId || '-')}</span>
            <span>→</span>
            <span>${escHtml(target.model || '-')}</span>
            ${backupCount ? `<span class="byok-route-more">+${backupCount} 备用</span>` : ''}
          </div>
          <div class="byok-slot-footer">
            <span class="byok-row-badge">${slot.supportsImages ? '图片' : escHtml(slotDisplayName(slot.modelUid))}</span>
            <div class="byok-inline-actions">
              <button class="byok-mini-btn" type="button" data-slot-action="failover" data-model-uid="${escAttr(slot.modelUid)}">故障转移</button>
              <button class="byok-mini-btn" type="button" data-slot-action="edit" data-model-uid="${escAttr(slot.modelUid)}">编辑</button>
              <button class="byok-mini-btn is-danger" type="button" data-slot-action="delete" data-model-uid="${escAttr(slot.modelUid)}">删除</button>
            </div>
          </div>
        </div>
      `;
  ...
  function collectByokSlotForm() {
    const modelUid = byokSelectedSlotUid || byokModelUid?.value || '';
    const providerId = byokSelectedSlotTarget?.providerId || byokSlotProvider?.value || '';
    const model = byokSelectedSlotTarget?.model || byokSlotModel?.value || '';
    const primary = providerId && model ? { providerId, model } : null;
    const existingTargets = byokNormalizeTargets(byokFindSlot(modelUid)?.targets);
    const targets = primary
      ? [primary, ...existingTargets.filter(target => target.providerId !== primary.providerId || target.model !== primary.model)]
      : [];
    const ideModel = getByokIdeModelList().find(item => item.modelUid === modelUid);
    return {
      modelUid,
      displayName: byokDisplayName?.value || '',
      supportsImages: ideModel?.supportsImages,
      enabled: byokSlotEnabled ? byokSlotEnabled.checked : true,
      targets,
    };
  }

  function saveByokSlotFromEditor() {
    const slot = collectByokSlotForm();
    if (!slot.modelUid) {
      setByokMsg(byokSlotMsg, '请选择 IDE 槽位', 'err');
      return;
    }
    if (!slot.targets.length) {
      setByokMsg(byokSlotMsg, '请选择映射目标模型', 'err');
      return;
    }
    setByokMsg(byokSlotMsg, '保存中...', '');
    postMsg('byokSaveSlot', { slot });
    closeByokSlotEditor();
    showToast('模型映射保存已提交', 'success', 1800);
  }

  function deleteByokSlot(uid) {
    const modelUid = uid || byokSlotEditUid?.value || byokSelectedSlotUid || byokModelUid?.value || '';
    if (!modelUid) return;
    postMsg('byokDeleteSlot', { modelUid });
    closeByokSlotEditor();
    setByokMsg(byokSlotMsg, '已提交删除', '');
  }

  function byokLogMatchesFilter(line, filter) {
    const text = String(line || '');
    if (!filter || filter === 'all') return true;
    if (filter === 'error') return /(error|failed|失败|错误|err|exception)/i.test(text);
    if (filter === 'warn') return /(warn|warning|警告|timeout|retry|重试)/i.test(text);
    if (filter === 'sidecar') return /sidecar|byok|pool|proxy/i.test(text);
    return true;
  }

  function renderByokLogs() {
    if (!byokLog) return;
    const stateLogs = byokState?.logs || [];
    const merged = uniqueStrings([...stateLogs, ...byokLocalLogs]).slice(-200);
    byokLocalLogs = merged.filter(line => !byokHiddenLogLines.has(line));
    const filter = byokLogFilter?.value || 'all';
    const visible = byokLocalLogs.filter(line => byokLogMatchesFilter(line, filter)).slice(-120);
    byokLog.textContent = visible.join('\n');
    byokLog.classList.toggle('is-empty', visible.length === 0);
    if (byokLogCount) {
      byokLogCount.textContent = filter === 'all'
        ? `${byokLocalLogs.length} 条`
        : `${visible.length} / ${byokLocalLogs.length} 条`;
    }
    byokLog.scrollTop = byokLog.scrollHeight;
  }

  function renderByokStats(stats) {
    const hasStats = !!stats;
    if (byokStatRequests) byokStatRequests.textContent = hasStats ? byokNumber(stats.requests) : '-';
    if (byokStatTokens) byokStatTokens.textContent = hasStats ? byokNumber(stats.totalTokens) : '-';
    if (byokStatRetries) byokStatRetries.textContent = hasStats ? byokNumber(stats.retries) : '-';
    if (byokStatErrors) byokStatErrors.textContent = hasStats ? byokNumber(stats.errors) : '-';
    if (byokStatLastRoute) {
      const route = hasStats && (stats.lastProvider || stats.lastModel)
        ? `最近路由 ${stats.lastProvider || '-'} / ${stats.lastModel || '-'}`
        : '最近路由 -';
      byokStatLastRoute.textContent = route;
    }
    if (byokStatRate) {
      const perMin = stats?.rate?.rpm;
      byokStatRate.textContent = Number.isFinite(perMin) ? `速率 ${perMin.toFixed(1)}/min` : '速率 -';
    }
  }
  ...
  function updateByokState(state) {
    ...
    const running = !!state.running;
    if (byokStatusPill) {
      byokStatusPill.textContent = running ? '运行中' : '未启动';
      byokStatusPill.className = 'byok-overview-status ' + (running ? '' : 'is-stopped');
    }
    setByokStatusValue(byokProxyStatus, running ? ('运行中' + (state.processId ? ' PID ' + state.processId : '')) : '未启动', running ? 'ok' : 'warn');
    const patch = state.patch || {};
    const patchOk = !!(patch.patchedApi && patch.patchedRestart);
    const patchParts = [
      patch.patchedApi ? 'API' : '',
      patch.patchedRestart ? 'LS' : '',
      patch.patchedInference ? 'INF' : '',
    ].filter(Boolean);
    if (byokCountPatches) byokCountPatches.textContent = patchParts.length + '/3';
    setByokStatusValue(byokPatchStatus, patchOk ? ('已应用 ' + patchParts.join('/')) : (patch.exists ? '未完整应用' : '未找到目标'), patchOk ? 'ok' : 'warn');
    setByokStatusValue(byokPortStatus, 'API ' + state.apiPort + ' / INF ' + state.inferencePort, '');
    renderByokStats(state.stats || null);
    if (byokStartBtn) byokStartBtn.disabled = running;
    if (byokStopBtn) byokStopBtn.disabled = !running;
    if (!byokSelectedProviderId && state.providers && state.providers.length) {
      byokSelectedProviderId = state.providers[0].id;
    }
    renderByokProviders();
    renderByokIdeModels();
    renderByokSlotProviderOptions();
    renderByokSlots();
    renderByokLogs();
    if (!byokProviderModal?.hidden) {
      ensureProviderCatalog();
      renderProviderSelectedModels();
      renderProviderModelCatalog();
    }
    if (!byokSlotModal?.hidden) {
      updateSlotHiddenFields();
      renderSlotCatalog();
      renderMappingModelCatalog();
    }
    if (!byokFailoverModal?.hidden) {
      renderFailoverRows();
    }
  ...
  function initByokPanel() {
    if (!byokStatusPill) return;
    const byokSubtabs = document.querySelectorAll('#byokSubtabs .byok-subtab');
    const byokContents = document.querySelectorAll('#tab-byok .byok-subtab-content');
    byokSubtabs.forEach(tab => {
      tab.addEventListener('click', () => {
        const key = tab.dataset.byokSubtab || 'providers';
        byokSubtabs.forEach(item => item.classList.toggle('active', item === tab));
        byokContents.forEach(content => {
          const active = content.dataset.byokContent === key;
          content.classList.toggle('active', active);
          if (active) content.removeAttribute('hidden');
          else content.setAttribute('hidden', '');
        });
    ...
    if (byokStartBtn) byokStartBtn.addEventListener('click', () => postMsg('byokStart'));
    if (byokStopBtn) byokStopBtn.addEventListener('click', () => postMsg('byokStop'));
    if (byokRefreshBtn) byokRefreshBtn.addEventListener('click', () => postMsg('byokLoad'));
    if (byokApplyPatchBtn) byokApplyPatchBtn.addEventListener('click', () => postMsg('byokApplyPatch'));
    if (byokRestorePatchBtn) byokRestorePatchBtn.addEventListener('click', () => postMsg('byokRestorePatch'));
    if (byokNewProviderBtn) byokNewProviderBtn.addEventListener('click', () => openByokProviderEditor());
    if (byokProviderBackBtn) byokProviderBackBtn.addEventListener('click', closeByokProviderEditor);
    if (byokProviderCancelBtn) byokProviderCancelBtn.addEventListener('click', closeByokProviderEditor);
    if (byokProviderFormat) byokProviderFormat.addEventListener('change', () => {
      if (byokProviderPath && (!byokProviderPath.value || byokProviderPath.value === '/v1/messages' || byokProviderPath.value === '/v1/chat/completions')) {
        byokProviderPath.value = byokDefaultPath(byokProviderFormat.value);
      }
      ensureProviderCatalog();
      renderProviderModelCatalog();
    });
    if (byokProviderList) {
      byokProviderList.addEventListener('click', async (e) => {
        const actionBtn = e.target.closest('[data-provider-action]');
        const card = e.target.closest('[data-provider-id]');
        const id = actionBtn?.dataset.providerId || card?.dataset.providerId || '';
        if (!id) return;
        const action = actionBtn?.dataset.providerAction || 'edit';
        if (action === 'edit') openByokProviderEditor(id);
        else if (action === 'test') testByokProvider(id);
        else if (action === 'toggle') {
          const provider = byokProviderById(id);
          if (provider) postMsg('byokSaveProvider', { provider: { ...provider, enabled: provider.enabled === false } });
        } else if (action === 'delete') {
          const ok = await showConfirm('确定删除该供应商？相关模型映射中的目标也会被移除。', '删除供应商');
          if (ok) postMsg('byokDeleteProvider', { providerId: id });
        } else {
          openByokProviderEditor(id);
        }
      });
    }
    if (byokProviderSelectedModels) {
      byokProviderSelectedModels.addEventListener('click', (e) => {
        const row = e.target.closest('[data-provider-model]');
        const action = e.target.closest('[data-provider-model-action]')?.dataset.providerModelAction;
        if (!row || !action) return;
        const model = row.dataset.providerModel || '';
        if (action === 'remove') removeProviderDraftModel(model);
        if (action === 'default') setProviderDraftDefault(model);
      });
    }
    if (byokProviderModelCatalog) {
      byokProviderModelCatalog.addEventListener('click', (e) => {
        const row = e.target.closest('[data-provider-catalog-model]');
        if (!row) return;
        addProviderDraftModel(row.dataset.providerCatalogModel || '');
      });
    }
    if (byokProviderModelFilters) {
      byokProviderModelFilters.addEventListener('click', (e) => {
        const btn = e.target.closest('button[data-filter]');
        if (!btn) return;
        byokProviderModelFilter = btn.dataset.filter || 'all';
        byokProviderModelFilters.querySelectorAll('button').forEach(item => item.classList.toggle('active', item === btn));
        renderProviderModelCatalog();
      });
    }
    if (byokProviderModelSearch) byokProviderModelSearch.addEventListener('input', renderProviderModelCatalog);
    if (byokProviderAddCustomModelBtn) byokProviderAddCustomModelBtn.addEventListener('click', () => {
      addProviderDraftModel(byokProviderCustomModel?.value || '');
      if (byokProviderCustomModel) byokProviderCustomModel.value = '';
    });
    if (byokProviderCustomModel) byokProviderCustomModel.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') {
        e.preventDefault();
        addProviderDraftModel(byokProviderCustomModel.value || '');
        byokProviderCustomModel.value = '';
      }
    });
    if (byokSaveProviderBtn) byokSaveProviderBtn.addEventListener('click', saveByokProviderFromEditor);
    if (byokTestProviderBtn) byokTestProviderBtn.addEventListener('click', () => testByokProvider());
    if (byokFetchModelsBtn) byokFetchModelsBtn.addEventListener('click', () => testByokProvider());
    if (byokDeleteProviderBtn) {
      byokDeleteProviderBtn.addEventListener('click', async () => {
        const id = byokProviderId?.value || byokSelectedProviderId;
        if (!id) return;
        const ok = await showConfirm('确定删除该供应商？相关模型映射中的目标也会被移除。', '删除供应商');
        if (!ok) return;
        postMsg('byokDeleteProvider', { providerId: id });
        closeByokProviderEditor();
      });
    }
    if (byokNewSlotBtn) byokNewSlotBtn.addEventListener('click', () => openByokSlotEditor());
    if (byokOpenMapSettingsBtn) byokOpenMapSettingsBtn.addEventListener('click', openByokMapSettings);
    if (byokOpenInjectedBtn) byokOpenInjectedBtn.addEventListener('click', openByokInjectedEditor);
    if (byokMapSettingsBackBtn) byokMapSettingsBackBtn.addEventListener('click', closeByokMapSettings);
    if (byokMapSettingsCancelBtn) byokMapSettingsCancelBtn.addEventListener('click', closeByokMapSettings);
    if (byokMapSettingsSaveBtn) byokMapSettingsSaveBtn.addEventListener('click', saveByokMapSettings);
    if (byokNamePrefix) byokNamePrefix.addEventListener('input', renderByokTemplatePreview);
    if (byokLabelTemplate) byokLabelTemplate.addEventListener('input', renderByokTemplatePreview);
    if (byokModelSearchInput) byokModelSearchInput.addEventListener('input', renderByokSlots);
    if (byokModelRefreshBtn) byokModelRefreshBtn.addEventListener('click', renderByokSlots);
    if (byokSlotBackBtn) byokSlotBackBtn.addEventListener('click', closeByokSlotEditor);
    if (byokSlotCancelBtn) byokSlotCancelBtn.addEventListener('click', closeByokSlotEditor);
    if (byokSlotCatalogSearch) byokSlotCatalogSearch.addEventListener('input', renderSlotCatalog);
    if (byokMappingModelSearch) byokMappingModelSearch.addEventListener('input', renderMappingModelCatalog);
    if (byokSlotCatalog) {
      byokSlotCatalog.addEventListener('click', (e) => {
        const row = e.target.closest('[data-slot-uid]');
        if (!row || row.classList.contains('disabled')) return;
        if (byokSlotEditUid?.value) return;
        byokSelectedSlotUid = row.dataset.slotUid || '';
        updateSlotHiddenFields();
        renderSlotCatalog();
      });
    }
    if (byokMappingModelCatalog) {
      byokMappingModelCatalog.addEventListener('click', (e) => {
        const row = e.target.closest('[data-target-provider][data-target-model]');
        if (!row) return;
        const provider = byokProviderById(row.dataset.targetProvider);
        byokSelectedSlotTarget = {
          providerId: row.dataset.targetProvider || '',
          providerName: provider?.name || row.dataset.targetProvider || '',
          model: row.dataset.targetModel || ''
        };
        updateSlotHiddenFields();
        renderMappingModelCatalog();
      });
    ...
    if (byokSlotList) {
      byokSlotList.addEventListener('click', async (e) => {
        const actionBtn = e.target.closest('[data-slot-action]');
        const row = e.target.closest('[data-model-uid]');
        const uid = actionBtn?.dataset.modelUid || row?.dataset.modelUid || '';
        if (!uid) return;
        const action = actionBtn?.dataset.slotAction || 'edit';
        if (action === 'delete') {
          const ok = await showConfirm('确定删除该模型映射？', '删除映射');
          if (ok) deleteByokSlot(uid);
        } else if (action === 'failover') {
          openByokFailoverEditor(uid);
        } else {
          openByokSlotEditor(uid);
        }
      });
    }
    if (byokFailoverBackBtn) byokFailoverBackBtn.addEventListener('click', closeByokFailoverEditor);
    if (byokFailoverCancelBtn) byokFailoverCancelBtn.addEventListener('click', closeByokFailoverEditor);
    if (byokFailoverAddBtn) byokFailoverAddBtn.addEventListener('click', addFailoverRow);
    if (byokFailoverSaveBtn) byokFailoverSaveBtn.addEventListener('click', saveByokFailover);
    if (byokFailoverRows) {
      byokFailoverRows.addEventListener('change', (e) => {
        const row = e.target.closest('[data-failover-index]');
        const field = e.target.closest('[data-failover-field]')?.dataset.failoverField;
        if (!row || !field) return;
        const idx = Number(row.dataset.failoverIndex);
        if (!Number.isFinite(idx) || !byokFailoverDraft[idx]) return;
        byokFailoverDraft[idx][field] = e.target.value || '';
        if (field === 'providerId') {
          const provider = byokProviderById(e.target.value);
          byokFailoverDraft[idx].model = byokProviderModels(provider)[0] || '';
        }
        renderFailoverRows();
      });
      byokFailoverRows.addEventListener('click', (e) => {
        const btn = e.target.closest('[data-failover-action]');
        const row = e.target.closest('[data-failover-index]');
        if (!btn || !row) return;
        const idx = Number(row.dataset.failoverIndex);
        const action = btn.dataset.failoverAction;
        if (!Number.isFinite(idx)) return;
        if (action === 'remove') byokFailoverDraft.splice(idx, 1);
        if (action === 'up' && idx > 0) [byokFailoverDraft[idx - 1], byokFailoverDraft[idx]] = [byokFailoverDraft[idx], byokFailoverDraft[idx - 1]];
        if (action === 'down' && idx < byokFailoverDraft.length - 1) [byokFailoverDraft[idx + 1], byokFailoverDraft[idx]] = [byokFailoverDraft[idx], byokFailoverDraft[idx + 1]];
        renderFailoverRows();
      });
    }
    if (byokInjectedBackBtn) byokInjectedBackBtn.addEventListener('click', closeByokInjectedEditor);
    if (byokInjectedCancelBtn) byokInjectedCancelBtn.addEventListener('click', closeByokInjectedEditor);
    if (byokInjectedSaveBtn) byokInjectedSaveBtn.addEventListener('click', saveByokInjectedEditor);
    if (byokInjectedSearch) byokInjectedSearch.addEventListener('input', renderInjectedEditor);
    if (byokInjectedFilter) byokInjectedFilter.addEventListener('change', renderInjectedEditor);
    if (byokInjectedList) {
      byokInjectedList.addEventListener('change', (e) => {
        const row = e.target.closest('[data-injected-uid]');
        if (!row) return;
        const uid = row.dataset.injectedUid || '';
        const model = getByokCatalogList().find(item => item.modelUid === uid);
        if (!model) return;
        const toggle = e.target.closest('[data-injected-action="toggle"]');
        const field = e.target.closest('[data-injected-field]')?.dataset.injectedField;
        if (toggle) {
          setInjectedDraftEnabled(model, !!toggle.checked);
          renderInjectedEditor();
          return;
        }
        const draft = injectedDraftByUid(uid);
        if (!draft || !field) return;
        if (field === 'supportsImages') draft.supportsImages = !!e.target.checked;
        else draft[field] = e.target.value || '';
        if (field === 'providerId') {
          const provider = byokProviderById(draft.providerId);
          draft.model = byokProviderModels(provider)[0] || model.apiId || '';
        }
        renderInjectedEditor();
      });
    }
    if (byokLogFilter) byokLogFilter.addEventListener('change', renderByokLogs);
    if (byokExportLogBtn) {
      byokExportLogBtn.addEventListener('click', () => {
    ...
    if (byokClearLogBtn) byokClearLogBtn.addEventListener('click', () => {
      [...(byokState?.logs || []), ...byokLocalLogs].forEach(line => byokHiddenLogLines.add(line));
      byokLocalLogs = [];
      renderByokLogs();
    });
    if (byokSaveSlotBtn) byokSaveSlotBtn.addEventListener('click', saveByokSlotFromEditor);
    if (byokDeleteSlotBtn) {
      byokDeleteSlotBtn.addEventListener('click', async () => {
        const ok = await showConfirm('确定删除该模型映射？', '删除映射');
        if (ok) deleteByokSlot();
      });
    }
    document.addEventListener('keydown', (e) => {
      if (e.key !== 'Escape') return;
      if (byokProviderModal && !byokProviderModal.hidden) closeByokProviderEditor();
      if (byokSlotModal && !byokSlotModal.hidden) closeByokSlotEditor();
      if (byokMapSettingsModal && !byokMapSettingsModal.hidden) closeByokMapSettings();
      if (byokFailoverModal && !byokFailoverModal.hidden) closeByokFailoverEditor();
      if (byokInjectedModal && !byokInjectedModal.hidden) closeByokInjectedEditor();
    });
    if (!byokRefreshTimer) {
      byokRefreshTimer = setInterval(() => {
        if (byokState?.running) postMsg('byokLoad');
      }, 5000);
    }
    postMsg('byokLoad');
  }
  ...
  function init() {
    ...
    // 顶层 tab 面板固定展开；内部高级设置仍保留折叠。
    (function pinPrimaryDetailsOpen() {
      try { localStorage.removeItem('ws-pool-details-open'); } catch {}
      const ids = ['listDetails', 'enhanceDetails', 'instDetails', 'acDetails', 'asDetails'];
      ids.forEach(id => {
        const el = document.getElementById(id);
        if (!el) return;
        ...
        if (summary) {
          summary.addEventListener('click', (e) => {
            ...
            setTimeout(() => el.setAttribute('open', ''), 0);
          });
        ...
        el.addEventListener('toggle', () => {
          if (!el.open) requestAnimationFrame(() => el.setAttribute('open', ''));
        });
      });
    })();
