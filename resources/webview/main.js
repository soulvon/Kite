(function() {
  ...
  function switchAcTab(tab) {
    ...
    // 错误恢复核心：仅守护模式和长任务模式显示
    const errorRecoverySection = $('#acErrorRecoverySection');
    if (errorRecoverySection) {
      errorRecoverySection.style.display = (tab === 'guardian' || tab === 'long-task') ? '' : 'none';
    }
